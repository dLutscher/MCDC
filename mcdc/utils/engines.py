mcdcimport numpy as np
import torch
import torch.nn.functional as F
from ignite.engine import Engine
from mcdc.utils import interpolation
from mcdc.utils import evaluation
from mcdc.utils import images
from mcdc.utils.layers import adjust_learning_rate
from mcdc.utils.utils import L2, lerp, swap_halves
from mcdc.utils.clustering import vade_cluster_acc
...
def create_mcdc_trainer(models, cfg):
    models['encoder'].to(cfg.MODEL.DEVICE)
    models['decoder'].to(cfg.MODEL.DEVICE)
    models['classifier'].to(cfg.MODEL.DEVICE)
    ...
    def _step(engine, batch):
        ...
        mixing_consistency_loss = F.mse_loss(out_mix, x) * cfg.MODEL.ADDED_CONSTR
        loss_ae_l2 = L2(disc_mix) * cfg.MODEL.ADVWEIGHT
        loss_ae = loss_ae_mse + mixing_consistency_loss + loss_ae_l2
        models['opt_ae'].zero_grad()
        loss_ae.backward(retain_graph=True)
        models['opt_ae'].step()
        ...
        losses['loss_ae_two'] = mixing_consistency_loss.item()
...
def create_idec_mcdc_finetuning_trainer(models, cfg):
    models['encoder'].to(cfg.MODEL.DEVICE)
    models['decoder'].to(cfg.MODEL.DEVICE)
    models['clustering'].to(cfg.MODEL.DEVICE)
    models['classifier'].to(cfg.MODEL.DEVICE)
    ...
    def _step(engine, batch):
        ...
        # 2. backprop
        loss_ae_mse = F.mse_loss(out, x)
        mixing_consistency_loss = F.mse_loss(out_mix, x)
        loss_ae_l2 = L2(disc_mix)

        loss_clust = F.kl_div(q.log(), models['clustering'].p[idx]) * cfg.MODEL.IDEC.GAMMA
        loss_ae = loss_ae_mse + loss_clust + mixing_consistency_loss + loss_ae_l2
...
def create_vae_mcdc_trainer(models, custom_loss, cfg):
    models['encoder'].to(cfg.MODEL.DEVICE)
    models['decoder'].to(cfg.MODEL.DEVICE)
    models['classifier'].to(cfg.MODEL.DEVICE)
    models['discriminator'].to(cfg.MODEL.DEVICE)
